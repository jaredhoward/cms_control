class Admin::CmsPagesController < Admin::BaseController
  ...
  def index
    @cms_pages = TitanControl::CmsPage.meta_title_order
  end

  def new
    @cms_page = TitanControl::CmsPage.new
    @cms_page.build_cms_meta
    1.times { @cms_page.cms_meta.cms_contents.build }
  end

  def create
    @cms_page = TitanControl::CmsPage.new(params[:cms_page])
    if @cms_page.save
  ...
  def load_cms_page
    @cms_page = TitanControl::CmsPage.find(params[:id])
  end
