module TitanControl
  module CmsHelper
    ...
    def url_for_meta(meta)
      meta = meta.id if meta.is_a?(TitanControl::CmsMeta)
      send(:"cms_#{meta}_path", {:format => 'html'})
    end

    def url_for_downloads(file_resource)
      file_resource = file_resource.id if file_resource.is_a?(TitanControl::FileResource)
    ...
    def block_for_product(product)
      ...
      product.file_resources.public_access.each do |file_resource|
        ...
        item << tag(:br)
        item << link_to('More Info', url_for_meta(file_resource.cms_meta.id))
    ...
    def build_top_nav
      ...
      pages.each do |page|
        page_url = page.cms_meta.url == 'home' ? root_url : url_for_meta(page.cms_meta.id)
        current_nav = {
